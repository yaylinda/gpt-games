...
begin
    ...
    SELECT
        ...
        true
    FROM
        (SELECT initial_chat_credits FROM public.admin_settings LIMIT 1) AS subquery;
...
CREATE FUNCTION "public"."handle_new_user"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$-- triggered when new auth.users row is inserted when supabase.auth.signUp is called
begin
...
CREATE FUNCTION "public"."search_messages"("chat_id" "uuid", "query_embedding" "public"."vector", "similarity_threshold" double precision, "max_rows" integer, "exclude_id" "uuid") RETURNS SETOF "public"."chat_messages"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  RETURN QUERY select * from (
    ...
    WHERE
      (
        ...
        AND
        1 - (chat_messages.embedding <=> search_messages.query_embedding) > search_messages.similarity_threshold
        AND 
        chat_messages.timestamp < (select timestamp from chat_messages cm1 where cm1.id = search_messages.exclude_id limit 1)
      ) 
    ORDER BY
      1 - (chat_messages.embedding <=> search_messages.query_embedding) DESC
    LIMIT search_messages.max_rows
  ) cm order by cm.timestamp asc;
END;
$$;
...
CREATE TRIGGER "on_after_profiles_insert" AFTER INSERT ON "public"."profiles" FOR EACH ROW EXECUTE FUNCTION "public"."after_profiles_insert"();

CREATE TRIGGER "on_after_purchases_insert" AFTER INSERT ON "public"."purchases" FOR EACH ROW EXECUTE FUNCTION "public"."after_purchases_insert"();

CREATE TRIGGER "on_before_purchases_insert" BEFORE INSERT ON "public"."purchases" FOR EACH ROW EXECUTE FUNCTION "public"."before_purchases_insert"();

CREATE TRIGGER "on_chat_insert" AFTER INSERT ON "public"."chats" FOR EACH ROW EXECUTE FUNCTION "public"."handle_chat_created"();

CREATE TRIGGER "on_chat_message_insert" AFTER INSERT ON "public"."chat_messages" FOR EACH ROW EXECUTE FUNCTION "public"."handle_chat_message_sent"();

CREATE TRIGGER "on_friend_request_accepted" AFTER UPDATE ON "public"."friends" FOR EACH ROW EXECUTE FUNCTION "public"."handle_friend_request_accepted"();
...
CREATE POLICY "Enable insert for users for self or chat" ON "public"."purchases" FOR INSERT WITH CHECK ((("auth"."uid"() = "user_id") OR ("auth"."uid"() IN ( SELECT "cm"."user_id"
   FROM "public"."chat_members" "cm"
  WHERE ("cm"."chat_id" = "purchases"."chat_id")))));

CREATE POLICY "Enable insert if user is creator and has sufficient chat credit" ON "public"."chats" FOR INSERT WITH CHECK ((("auth"."uid"() = "created_by") AND (EXISTS ( SELECT "p"."id",
    "p"."created_at",
    "p"."username",
    "p"."discriminator",
  ...
  WHERE (("p"."num_chat_credits_used" < "p"."num_chat_credits_total") AND ("p"."id" = "auth"."uid"()))))));

CREATE POLICY "Enable insert if user is part of chat and sufficient credits" ON "public"."chat_messages" FOR INSERT WITH CHECK ((("auth"."uid"() IN ( SELECT "cm"."user_id"
   FROM "public"."chat_members" "cm"
  WHERE ("cm"."chat_id" = "chat_messages"."chat_id"))) AND ((( SELECT "c1"."num_message_credits_used"
   FROM "public"."chats" "c1"
  WHERE ("c1"."id" = "chat_messages"."chat_id")) < ( SELECT "c2"."num_message_credits_total"
   FROM "public"."chats" "c2"
  WHERE ("c2"."id" = "chat_messages"."chat_id"))) OR ("sender_id" IS NULL))));

CREATE POLICY "Enable read for users for self or chat" ON "public"."purchases" FOR SELECT USING ((("auth"."uid"() = "user_id") OR ("auth"."uid"() IN ( SELECT "cm"."user_id"
   FROM "public"."chat_members" "cm"
  WHERE ("cm"."chat_id" = "purchases"."chat_id")))));

CREATE POLICY "Enable select if user in chat" ON "public"."chats" FOR SELECT USING (("auth"."uid"() = ANY ("members")));

CREATE POLICY "Enable update for users for self or chat" ON "public"."purchases" FOR UPDATE USING ((("auth"."uid"() = "user_id") OR ("auth"."uid"() IN ( SELECT "cm"."user_id"
   FROM "public"."chat_members" "cm"
  WHERE ("cm"."chat_id" = "purchases"."chat_id"))))) WITH CHECK ((("auth"."uid"() = "user_id") OR ("auth"."uid"() IN ( SELECT "cm"."user_id"
   FROM "public"."chat_members" "cm"
  WHERE ("cm"."chat_id" = "purchases"."chat_id")))));

CREATE POLICY "Enable update if user in chat" ON "public"."chats" FOR UPDATE USING (("auth"."uid"() IN ( SELECT "cm"."user_id"
   FROM "public"."chat_members" "cm"
  WHERE ("cm"."chat_id" = "chats"."id")))) WITH CHECK (("auth"."uid"() IN ( SELECT "cm"."user_id"
   FROM "public"."chat_members" "cm"
  WHERE ("cm"."chat_id" = "chats"."id"))));

CREATE POLICY "Enable update if user is part of chat" ON "public"."chat_messages" FOR UPDATE USING (("auth"."uid"() IN ( SELECT "cm"."user_id"
   FROM "public"."chat_members" "cm"
...
CREATE POLICY "Users can update own or friends profile." ON "public"."profiles" FOR UPDATE USING ((("auth"."uid"() = "id") OR (EXISTS ( SELECT 1
   FROM "public"."friends" "f"
  WHERE ((("f"."requester" = "auth"."uid"()) AND ("f"."requestee" = "profiles"."id")) OR (("f"."requestee" = "auth"."uid"()) AND ("f"."requester" = "profiles"."id")))))));

CREATE POLICY "Users can view chat messages where they are recipients" ON "public"."chat_messages" FOR SELECT USING (("auth"."uid"() IN ( SELECT "cm"."user_id"
   FROM "public"."chat_members" "cm"
  WHERE ("cm"."chat_id" = "cm"."chat_id"))));
